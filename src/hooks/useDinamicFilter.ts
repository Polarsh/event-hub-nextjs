...
export const useDynamicFilters = () => {
  ...
  const handleFilterChange = useCallback(
    (filterId: number, childrenId: null) => {
      setActiveFilters((prevFilters: any[]) => {
        console.log('filterId', filterId, 'childrenId', childrenId)
        ...
        if (childrenId === null) {
          ...
          if (categoryInInitialFilters) {
            ...
            const newCategory = {
              ...
              children: categoryInInitialFilters.children.map(
                (child: { id: any }) => ({
                  id: child.id,
                })
              ),
        ...
        const existingCategory = prevFilters.find(
          (f: { id: any }) => f.id === filterId
        )

        if (existingCategory) {
          const newFilters = prevFilters
            .map((f: { id: any; children: any[] }) => {
              if (f.id === filterId) {
                const childExists = f.children.find(
                  (c: { id: any }) => c.id === childrenId
                )
  ...
  const handleSortChange = useCallback((value: string) => {
    ...
    setActiveSorts((prevSorts: any[]) => {
      // Check if the new selection is already in the array
      const isAlreadyActive = prevSorts.some(
        (sort: { value: any }) => sort.value === value
      )
      if (isAlreadyActive) {
        // Remove the item if it's already active (toggle off)
        return prevSorts.filter((sort: { value: any }) => sort.value !== value)
      }

      let newSorts = [...prevSorts]

      // Mutually exclusive rule for 'exclusive' type
      if (newSortItem.type === 'exclusive') {
        newSorts = newSorts.filter(sort => sort.type !== 'exclusive')
      } else {
      ...
      // Enforce max of 2 items by removing the oldest
      if (newSorts.length > 2) {
        newSorts.shift() // Remove the first (oldest) item
      }
  ...
  const isFilterActive = useCallback(
    (filterId: any, childrenId: any) => {
      const category = activeFilters.find((f: { id: any }) => f.id === filterId)
      return category
        ? category.children.some((c: { id: any }) => c.id === childrenId)
        : false
    },
    [activeFilters]
  )
  ...
  const isSortActive = useCallback(
    (value: any) =>
      activeSorts.some((sort: { value: any }) => sort.value === value),
    [activeSorts]
  )
